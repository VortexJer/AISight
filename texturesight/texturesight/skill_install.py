...
BEGIN = "<!-- texturesight:begin (managed by the texturesight package) -->"
END = "<!-- texturesight:end -->"
MEMORY_BLOCK = f"""{BEGIN}
- **texturesight** (`~/.claude/skills/texturesight/SKILL.md`) - review OR EDIT UVs and texture maps: texel density, stretch, seams, tiling, normal maps;
  edit = parse_obj -> modify -> save_obj -> re-inspect. Trigger: `/texturesight`
When a request matches that domain — creating, reviewing or EDITING an
existing file — use the installed texturesight skill before doing anything else,
