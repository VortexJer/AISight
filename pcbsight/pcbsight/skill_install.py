...
BEGIN = "<!-- pcbsight:begin (managed by the pcbsight package) -->"
END = "<!-- pcbsight:end -->"
MEMORY_BLOCK = f"""{BEGIN}
- **pcbsight** (`~/.claude/skills/pcbsight/SKILL.md`) - review OR EDIT PCB layouts (.kicad_pcb): open nets, clearances, trace current, diff pairs;
  edit = load_sexpr -> modify -> save_sexpr -> re-inspect. Trigger: `/pcbsight`
When a request matches that domain — creating, reviewing or EDITING an
existing file — use the installed pcbsight skill before doing anything else,
